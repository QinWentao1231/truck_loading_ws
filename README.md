...
| 数据块 | 内容 |
| --- | --- |
| 1 | 当前 block 总箱数、码垛面数、车厢宽度、`head.W`、`head.L` |
...
第1块的前5个浮点依次为：总箱数、码垛面数、车厢宽度、异形车头前端
宽度 `head.W`、异形车头长度 `head.L`。未配置有效的车头尺寸时，最后
两个位置发送 `0.0`。该扩展不增加数据块数量，也不改变单块41字节长度。

### `cmd_get_per_count`

当前返回1个数据块，前4个浮点依次为：当前面放置次数、当前箱型、底层
每行抓数、异形车头角点单侧收缩量（mm）。单侧收缩量按
`(original.W - 当前面可用车宽) / 2` 计算；超过异形车头区域后固定为
`0.0`。每次请求均从当前 `boxes[0]` 的 `is_head` 和 `car_width` 字段现场
计算，队列为空时才回退到下一条待执行路径，不依赖路径任务的 `action`
处理时序。重复请求本命令不会推进码垛面，也不会重复累计收缩量。
