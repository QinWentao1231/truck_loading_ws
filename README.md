...
| 数据块 | 内容 |
| --- | --- |
| 1 | 当前 block 总箱数、码垛面数、车厢宽度、`head.W`、`head.L`、尾门门框宽度 `frame.W` |
| 2 | 当前 block 默认箱型的有效尺寸 `L/W/H` |
| 3 | 前6个混装 block 在完整 `rp_list` 中的位置，不足6个补0 |
| 4 | 前6个异形车头 block 在完整 `rp_list` 中的位置，不足6个补0 |

混装 block 位置从 `1` 开始编号：不存在混装 block 时全部为 `0`，`rp_list[0]` 是混装 block 时为 `1`，`rp_list[1]` 是混装 block 时为 `2`。最多发送前6个位置。

异形车头 block 使用相同的1起始编号规则。除下一层垛型条目的显式
`Ishead=true` 外，`CarCondition.head` 有效时也会根据累计纵深自动识别；
两种方式均未识别到异形区域时，第4块全部为 `0`。

第1块的前6个浮点依次为：总箱数、码垛面数、车厢宽度、异形车头前端
宽度 `head.W`、异形车头长度 `head.L`、尾门门框宽度 `frame.W`。对应
尺寸未配置时发送 `0.0`。该扩展不增加数据块数量，也不改变单块41字节长度。
...
| 数据块 | float槽位 | 内容 |
| --- | --- | --- |
| 1 | `float[0]` | 当前面全部P1/P2/P3动作的总放置抓数 |
| 1 | `float[1]` | 当前待取抓的箱型代号；混装面可随抓变化 |
| 1 | `float[2]` | 当前面P1最低层的单行抓数，不是单行箱数 |
| 1 | `float[3]` | 异形车头左右角点各自向内收缩的距离，单位mm |
| 1 | `float[4:9]` | 保留槽位，固定为0 |
...
第1块的`box_cfg`是数量编码：非混装面的1XX箱型P1行恰好三抓时，物理
最右抓发送“实际箱数+20”；常规垛和梯形垛均适用，该规则优先于原有的
不可翻转编码。其他抓中，20x/30x箱型任意区域以及10x箱型P3区域发送
“实际箱数+10”，其余发送实际箱数。
