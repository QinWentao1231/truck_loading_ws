...
| `mixture_side_brace_min_bottom_ratio` | 启用侧向限位时要求的最低底部支撑率，默认 0.35 |
| `mixture_side_brace_max_gap_mm` | 倾倒侧箱体与已码侧挡箱之间允许的最大间隙，单位 mm，默认 20 |
| `mixture_side_brace_min_x_overlap_ratio` | 侧挡箱与当前箱沿车深X方向的最小重叠比例，必须严格大于该值，默认 0.6 |
| `mixture_side_brace_min_z_overlap_ratio` | 侧挡箱与当前箱沿高度Z方向的最小重叠比例，必须严格大于该值，默认 0.45 |
...
执行`cmd_chk_path`时，混装面还会按实际放置顺序检查每一抓的底面支撑情况。
该检查只告警和记录，不会阻止路径下发。当前风险判据是单箱重心是否落在直接
接触面内，或至少两个支撑面的联合凸包内。重心沿车宽Y方向越出底部支撑区时，
若底部支撑率不低于35%，倾倒侧存在已经稳定的箱体，并且侧面间隙不超过20mm、
X方向重叠严格超过60%、Z方向重叠严格超过45%，则记为`side_braced`侧向限位
稳定，不再作为路径异常。完全悬空、沿X方向失稳或侧挡条件不足仍会报警。
`mixture_support_min_ratio`和`mixture_support_min_box_ratio`只作为诊断信息，
不单独触发告警。整抓支撑率、最低单箱支撑率、侧向限位详情和风险单箱编号会
写入检查汇总；面级PNG使用红色实线框出风险抓，并以粗红底边标记不稳定单箱。
...
| 条件 | 发送值 |
| --- | --- |
| `conveyor_left_transfer_enable=true`，且为非混装、非尾门的1XX箱型P1行，一行恰好9箱分3抓，并且是物理最右抓 | 实际箱数+20；常规垛和非尾门梯形垛均适用 |
| 2XX或3XX箱型的任意区域 | 实际箱数+10 |
| 1XX箱型的P3区域 | 实际箱数+10 |
| 其他情况 | 实际箱数 |

“实际箱数+20”的优先级高于“实际箱数+10”。物理最右抓按放置Y坐标
判断，不按执行顺序判断。尾门梯形仍按左、中、右顺序执行，并且明确不使用
+20编码；10箱三抓、混装面、P2、P3以及2XX/3XX也不会使用+20编码。
...
与`box_cfg`相互独立；尾门梯形P1简单行的`area_cfg`固定为1，同时保持原始
箱数编码，不使用+20。
...
| `float[2]` | `first_segment_box_count` | 当前抓第一段包含的箱数 | 多段放置时不是整抓总箱数 |
| `float[3]`～`float[8]` | 保留 | 当前未使用 | 0 |
